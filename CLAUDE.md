...
**Hero portrait** (`js/hero-photo.js`) animates the `<img id="scene">` in the hero stage — a background-removed photo (`assets/portrait.png`), duotone-treated via CSS `filter` to match the site's palette. It's a plain script, independent of `main.js`/`content.js`: idle drift, a subtle lean toward the pointer, and a scale-down as the page scrolls past it, all gated by `data-motion` exactly like the mechanism above. It also pauses via `IntersectionObserver` when the stage scrolls offscreen and on tab visibility change. (This replaced an earlier Three.js object; see git history if you need it back.)
